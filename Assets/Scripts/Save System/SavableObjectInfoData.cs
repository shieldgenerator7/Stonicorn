...
public struct SavableObjectInfoData
{
    ...
    public int destroyStateId;//the game state id in which this object was destroyed (max value for not destroyed)
    ...
    public SavableObjectInfoData(SavableObjectInfo soi)
    {
        ...
        if (this.destroyStateId < 0)
        {
            Debug.LogError("Destroy state id is invalid!: " + this.destroyStateId, soi.gameObject);
        }
    }
